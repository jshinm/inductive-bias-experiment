...
class FigGenerator:
    ...
    def plot_posterior(dset=0, h=0.01, enable=[1, 1, 1, 1], cc=False, savefile=True):
        ...
        for i in range(dsetnum):
            ...
            pcm = axs.scatter(
                X1[i], X2[i], c=get_colors(colors, y[i]), s=20)
        ...
        if savefile:
            plt.savefig('figs/' + str(datetime.today())
                        [:10] + '_simulated_datasets_' + '(' + str(h) + ').png')
        ...
        if savefile:
            plt.savefig('figs/' + str(datetime.today())
                        [:10] + '_true_posteriors_' + '(' + str(h) + ').png')
    ...
    def plot_posterior(dset=0, h=0.01, enable=[1, 1, 1, 1], cc=False, savefile=True):
        ...
        for j in range(dsetnum):
            ...
            for i in range(len(post[0])+2):
                ...
                if cnt % 9 == 1:
                    cnt3 += 1
                    pcm = axs.scatter(
                        X1[cnt3], X2[cnt3], c=get_colors(colors, y[cnt3]), s=10)
                elif cnt % 9 == 2:
                    cnt2 += 1
                    pcm = axs.scatter(
                        xx, yy, c=true_zz[cnt2], cmap=cmap, s=5)
                else:
                    pcm = axs.scatter(
                        uX0, uX1, c=post[j][i-2], cmap=cmap, s=5)
        ...
        if savefile:
            plt.savefig('figs/' + str(datetime.today())
                        [:10] + '_estimated_posterior_' + '(' + str(h) + ').png')
        ...
        for j in range(dsetnum):
            ...
            for k in range(row):
                for i in range(col):
                    ...
                    # simulation dataset
                    if cnt == 1:
                        # c=get_colors(colors, y[j]),
                        pcm = axs.scatter(
                            X1[j], X2[j], c=y[j], cmap=cmap, s=10)
                        if cc:
                            axs.add_patch(
                                Circle((0, 0), radius=1, linewidth=2, edgecolor='k', fill=False))
                        else:
                            axs.add_patch(Rectangle(
                                (-1, -1), 2, 2, linewidth=2, edgecolor='k', fill=False))
                    # true posterior
                        ...
                        if cc:
                            pcm = axs.scatter(
                                xx, yy, c=Ctrue_zz[j], cmap=cmap, s=1)
                            axs.add_patch(
                                Circle((0, 0), radius=1, linewidth=2, edgecolor='k', fill=False))
                        else:
                            pcm = axs.scatter(
                                xx, yy, c=true_zz[j], cmap=cmap, s=1)
                            axs.add_patch(Rectangle(
                                (-1, -1), 2, 2, linewidth=2, edgecolor='k', fill=False))
                    # estimated posterior
                        ...
                        if cc:
                            pcm = axs.scatter(
                                uX0, uX1, c=Cpost[j][i-1], cmap=cmap, s=1)
                            axs.add_patch(
                                Circle((0, 0), radius=1, linewidth=2, edgecolor='k', fill=False))
                        else:
                            pcm = axs.scatter(
                                uX0, uX1, c=post[j][i-1], cmap=cmap, s=1)
                            axs.add_patch(Rectangle(
                                (-1, -1), 2, 2, linewidth=2, edgecolor='k', fill=False))
                    # colorbar
                    # elif i == col-1:
                    ...
                    # hellinger
                        ...
                        if cc:
                            pcm = axs.scatter(
                                CuX_outside[:, 0], CuX_outside[:, 1], c=COUT_hellinger[j][i-1], cmap='binary', s=5)
                            axs.add_patch(
                                Circle((0, 0), radius=1, linewidth=2, edgecolor='k', fill=False, hatch='/'))
                            temptitle = 'Mean Distance:' + \
                                str(round(COUT_hellinger[j][i-1].mean(), 2))
                            axs.set_title(temptitle, fontsize=fsize)
                        else:
                            pcm = axs.scatter(
                                uX_outside[:, 0], uX_outside[:, 1], c=OUT_hellinger[j][i-1], cmap='binary', s=5)
                            axs.add_patch(Rectangle(
                                (-1, -1), 2, 2, linewidth=2, edgecolor='k', fill=False, hatch='/'))
                            temptitle = 'Mean Distance:' + \
                                str(round(OUT_hellinger[j][i-1].mean(), 2))
                            axs.set_title(temptitle, fontsize=fsize)
                    # fig.colorbar(pcm, ax=axs)
                    if k == 0 and i == 0:
                        axs.set_title('Simulation Data', fontsize=fsize)
            ...
            if savefile:
                if cc:
                    ...
                    path = 'figs/' + \
                        ...
                        '_' + data[j] + '_Cfigures.png'
                else:
                    # + '(' + str(h) + ')'
                    path = 'figs/' + \
                        str(datetime.today())[:10] + \
                        '_' + data[j] + '_figures.png'
    ...
    def figure1(dset=0, h=0.01, enable=[1, 1, 1, 1], cc=False, savefile=True):
        '''
            ...
            for k in range(row):
                for i in range(col):
                    ...
                    # estimated posterior
                        ...
                        if cnt == 7:
                            if j == 2:
                                ...
                                grid_near = griddata((MT_4[:, 3], MT_4[:, 5]), MT_4[:, 0], (
                                    uX0, uX1), method=interp_method, rescale=True)  # , fill_value=0.5
                                pcm = axs.scatter(
                                    uX0, uX1, c=grid_near, cmap=cmap, s=1)
                            elif j == 4:
                                # pcm = axs.scatter(MT_2[:,3],MT_2[:,5], c=MT_2[:,0], cmap=cmap, s=10)
                                grid_near = griddata(
                                    (MT_2[:, 3], MT_2[:, 5]), MT_2[:, 0], (uX0, uX1), method=interp_method)
                                pcm = axs.scatter(
                                    uX0, uX1, c=grid_near, cmap=cmap, s=1)
                            ...
                            pcm = axs.scatter(
                                uX0, uX1, c=Cpost[j][2], cmap=cmap, s=1)
                    ...
                    if cnt != 4 and cnt != 8:
                        ...
                        circle = Circle((0, 0), 3, linewidth=1,
                                        edgecolor='k', facecolor='none')
                        axs.add_patch(
                            Circle((0, 0), radius=1, linewidth=2, ls='--', edgecolor='r', fill=False))
                    ...
                    if cnt == 4:
                        ...
                        # different radius range
                        l2 = sns.lineplot(
                            human_i[-ii+1], human_p[-ii+1], ci=95)
                        lines.append(l2)
                        ...
                        # different radius range
                        l2 = sns.lineplot(
                            human_i[-ii+1], human_h[-ii+1], ci=95)
                        lines.append(l2)
                        axs.legend(lines, new_mods, prop={'size': 9})
...
###################
            ...
            if savefile:
                if cc:
                    # + '(' + str(h) + ')'
                    path = 'figs/' + \
                        str(datetime.today())[:10] + '_' + \
                        data[j] + '_Cfigures_human.png'
                else:
                    # + '(' + str(h) + ')'
                    path = 'figs/' + \
                        str(datetime.today())[:10] + '_' + \
                        data[j] + '_figures_human.png'
                plt.tight_layout(pad=2)
                plt.savefig(path, bbox_inches='tight')
